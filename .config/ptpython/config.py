...
def configure(repl):
    ...
    @repl.add_key_binding(Keys.ControlJ, filter=True)
    def execute_current_commands(event: KeyPressEvent):
        buff = event.current_buffer
        buff.validate_and_handle()

    @repl.add_key_binding(Keys.ControlP, filter=True)
    def history_backward(event: KeyPressEvent):
        event.current_buffer.history_backward()

    @repl.add_key_binding(Keys.ControlN, filter=True)
    def history_forward(event: KeyPressEvent):
        event.current_buffer.history_forward()

    @repl.add_key_binding(Keys.ControlQ, 'y', filter=True)
    def copy_to_clipboard(event: KeyPressEvent):
