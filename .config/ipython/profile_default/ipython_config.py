...
# c.TerminalInteractiveShell.banner1 = "Python 3.7.8 | packaged by conda-forge | (default, Jul 31 2020, 02:25:08) \nType 'copyright', 'credits' or 'license' for more information\nIPython 7.18.1 -- An enhanced Interactive Python. Type '?' for help.\n"
...
################################################################################
#                    Local changes/overrides by infokiller                     #
################################################################################
...
def _get_history_file():
    old_hist_file = os.path.expanduser(
        '~/.config/ipython/profile_default/history.sqlite')
    host_hist_dir = os.environ.get(
        'HOST_HIST_DIR',
        os.path.expanduser(
            os.path.join('~/.local/var/hist', socket.gethostname())))
    new_hist_file = os.path.join(host_hist_dir, 'ipython_hist.sqlite')
    if os.path.exists(old_hist_file):
        try:
            os.rename(old_hist_file, new_hist_file)
        except FileExistsError:
            print('Found both old and new hist files, fix manually')
    return new_hist_file
